# 简单记录

#### 1. 三方库
- 1.1 [三方库](./三方库.md)

#### 2. dagger2
- 2.11 [dagger2](./di/dagger/dagger.md)
- 2.12 [dagger2官方文档](https://dagger.dev/dev-guide/multibindings)

#### 3. hilt
- 3.11 [hilt](./di/hilt/hilt.md)
- 3.11 [hilt官方文档](https://dagger.dev/dev-guide/multibindings)

#### 4. 注解处理器
- 4.1 [apt](./apt/apt.md)
- 4.2 [javapoet](https://github.com/square/javapoet)

#### 5. 配网
- 5.0 [配网文档](./netconfig/netconfig.md)
- 5.1 [SoftAP配网](./netconfig/SoftAp.md)
- 5.2 [BLE配网](./netconfig/ble.md)
- 5.3 [SmartConfig/AirKiss配网](./netconfig/SmartConfig_AirKiss.md)

#### 6. 微信小程序配网
  - 6.1 [微信小程序softAP配网](https://github.com/Fimics/wechat_config)
  - 6.2 [小程序WiFi+蓝牙控制ESP32-C3配网](https://github.com/Fimics/Ai-Thinker-Open-WechatMini-ESP32-C3)
  - 6.3 [一款开源的蓝牙ble配网blufi乐鑫芯片esp32的开源微信小程序（持续维护)](https://github.com/Fimics/BlufiEsp32WeChat)
  - 6.4 [WeChatAirkisss](https://github.com/Fimics/WeChatAirkiss)

#### 7.语言
- 7.1 [kotlin-core](./kotlin/kotlin-core/kotlin.md)
- 7.2 [kotlin-coroutine](./kotlin/kotlin-coroutine/kotlin-coroutine.md)
- 7.3 [kotlin-java](./kotlin/kotlin-java/java.md)

#### 8. Rx Java android
- 8.1 [RxJava2](./rx/rx2/rxjava2.md)
- 8.2 [RxJava3](./rx/rx3/rxjava3.md)

#### 9. 字节码技术
- 9.1 [Javassist](./di/Javassist.md)
- 9.2 [Javassit ASM,bytecode](https://github.com/fuzhengwei/itstack-demo-bytecode)

#### 10. 蓝牙模块
- 10.1 [蓝牙模块](./ble/ble.md)

#### 11. android-python(Chaquopy)
- 11.1 [android-python(Chaquopy)](./extras/libpy/android-python.md)

#### 98.AppUpdate
- 98.1  [AppUpdate]()

#### 99. AI编程工具
- 99.1 Trae，qoder,GitHub Copilot,Cursor,Codeium,Tabnine

#### 100. 参考资料

#### 101. android studio 插件
- 101.1 通义lingma


#### 102. TODO
- 102.1 自动发现机制 通常配合 SPI 机制自动发现实现类： 使用 ServiceLoader
- 102.2 AutoDispose 库来绑定 RxJava 与 Android 生命周期 的工具类
- 102.3 RxJava2
    
