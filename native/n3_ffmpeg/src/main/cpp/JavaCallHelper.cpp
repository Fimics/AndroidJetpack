...
JavaCallHelper::JavaCallHelper(JavaVM *_javaVM, JNIEnv *_env, jobject &_jobj) : javaVM(_javaVM),
    ...
    jmid_video_size_change = env->GetMethodID(jclazz, "onVideoSizeChanged", "(II)V");
}
...
void JavaCallHelper::onVideoSizeChanged(int thread, int w, int h) {
    if (thread == THREAD_CHILD) {
        JNIEnv *jniEnv;
        if (javaVM->AttachCurrentThread(&jniEnv, nullptr) != JNI_OK) {
            return;
        }
        jniEnv->CallVoidMethod(jobj, jmid_video_size_change, w, h);
        javaVM->DetachCurrentThread();
    } else if (thread == THREAD_MAIN) {
        env->CallVoidMethod(jobj, jmid_video_size_change, w, h);
    }
}
