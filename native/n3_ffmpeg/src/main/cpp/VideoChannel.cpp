...
void *decodeVideo(void *args) {
    auto *p_video_channel = static_cast<VideoChannel *>(args);
    p_video_channel->javaCallHelper->onVideoSizeChanged(THREAD_CHILD, p_video_channel->avCodecContext->width, p_video_channel->avCodecContext->height);
    p_video_channel->decodePacket();        // 解码loop
    return nullptr;
}

void *synchronize(void *args) {
    auto *p_video_channel = static_cast<VideoChannel *>(args);
    p_video_channel->synchronizeFrame();        // 音视频同步输出画面
    return nullptr;
}
...
void VideoChannel::start() {
    ...
    LOG_D("线程pid_video_decode:%lu", pid_video_decode);
    // 开启视频播放线程
    pthread_create(&pid_synchronize, nullptr, synchronize, this);
    LOG_D("线程pid_synchronize:%lu", pid_synchronize);
}
...
void VideoChannel::synchronizeFrame() {
    ...
    SwsContext *sws_ctx = sws_getContext(// 需要释放 sws_freeContext
            ...
            AV_PIX_FMT_RGBA,// 输出算法和选项，RGBA8888，因为 SurfaceView支持
            SWS_BILINEAR, nullptr, nullptr, nullptr);       // libyuv效率更高一些 差不多提高一倍的性能
    ...
    while (is_playing) {
        ...
        double audioClock = p_audio_channel->clock;// 音频的播放时间
