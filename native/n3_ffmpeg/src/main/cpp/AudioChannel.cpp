...
void *decodeAudio(void *args) {     // 线程
    auto *p_audio_channel = static_cast<AudioChannel *>(args);
    p_audio_channel->decodePacket();    // 解码loop所在
    return nullptr;
}

void *audioPlay(void *args) {       //线程函数
    auto *p_audio_channel = static_cast<AudioChannel *>(args);
    p_audio_channel->initOpenSL();  // 播放loop所在
    return nullptr;
}
...
void bqPlayerCallback(SLAndroidSimpleBufferQueueItf bq, void *context) {
    ...
    if (data_len > 0) {
        // 添加到缓冲队列
        (*bq)->Enqueue(bq, audioChannel->out_buffer, data_len);     // 本身opensl se有延迟
    }
}
...
void AudioChannel::start() {
    ...
    is_playing = true;// TODO 播放完成需要设置false？
    // 开启音频解码线程
    pthread_create(&pid_audio_decode, nullptr, decodeAudio, this);
    LOG_D("线程pid_audio_decode:%lu", pid_audio_decode);
    // 开启初始化OpenSL ES线程
    pthread_create(&pid_audio_play, nullptr, audioPlay, this);
    LOG_D("线程pid_audio_play:%lu", pid_audio_play);
}

void AudioChannel::stop() {
    is_playing = 0;
    pkt_queue.setWork(0);       // 通知队列 要停止了
    frame_queue.setWork(0);
    pthread_join(pid_audio_decode,0);   // 等待解码线程退出
    pthread_join(pid_audio_play,0);     // 等待播放线程
    if(swr_ctx){
        swr_free(&swr_ctx);     // 释放重采样器
        swr_ctx = 0;
    }
    //释放播放器
    if(bqPlayerObject){
